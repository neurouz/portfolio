  ...
  repos.forEach((id) => {
    ...
    repo.then((result) => {
      ...
      var date_modified = new Date(
        result.data.updated_at.trim()
      ).toLocaleDateString();

      // Creating HTML elements
      var card = insertElement("div", {
        class: "project-card",
        "data-aos": "zoom-out-up",
        "data-aos-delay": dataDelay,
      });
      var p = insertElement("p", { class: "project-title" }, title);
      var img = insertElement("img", {
        src: image,
        class: "project-image",
        alt: "project-image",
      });
      var dateElement = insertElement(
        "p",
        { class: "project-date" },
        "Created: " + localDate
      );
      var last_modified = insertElement(
        "p",
        { class: "modified-date" },
        "Modified: " + date_modified
      );

      var visit = insertElement(
        "a",
        {
          class: "project-link",
          href: link,
      ...
      if (result.data.id == "306111171") {
        description =
          description +
          "<br />Credentials:<br /><b> Admin : AppAdmin1!<br />User : AppUser1!</b>";
      }
      ...
      if (result.data.id == "292325199") {
        var div = insertElement("div", {
          style:
            "display: flex; flex-direction: row; width: 50%; margin: auto; justify-content: space-around;",
        });

        var downloadMobile = insertElement(
          "a",
          {
        ...
        var downloadDesktop = insertElement(
          "a",
          {
        ...
        div.append(downloadMobile);
        div.append(downloadDesktop);
        card.append(div);
      }
